import { CREAM, GOLD, mono } from './posterKit.jsx'
        ...
        {label && (
          <g style={{ opacity: on ? 1 : 0, transition: 'opacity .6s .25s' }}>
            <line x1={anchor[0]} y1={anchor[1]} x2={labelAt[0]} y2={labelAt[1]}
              stroke={CREAM} strokeOpacity=".45" strokeWidth="0.8" strokeDasharray="2 3" />
            <circle cx={anchor[0]} cy={anchor[1]} r="1.6" fill={GOLD} />
            <text x={labelAt[0]} y={labelAt[1] - 3} fontFamily="'Space Mono', monospace" fontSize="9"
              fill={CREAM} fillOpacity=".75" textAnchor={labelAt[0] > 150 ? 'start' : 'end'}
...
const BUS = (solid) => (
  <g stroke={CREAM} strokeWidth={solid ? 1.6 : 1.2} fill={solid ? 'rgba(244,239,230,.06)' : 'none'}
    strokeDasharray={solid ? 'none' : '4 4'}>
    ...
    {solid && <>
      ...
      <rect x="113" y="157" width="14" height="14" strokeOpacity=".7" />
      <circle cx="178" cy="226" r="6" strokeOpacity=".7" />
    </>}
  </g>
)

const WING = (x, solid) => (
  <g stroke={CREAM} strokeWidth={solid ? 1.4 : 1} fill={solid ? 'rgba(201,162,39,.13)' : 'none'}
    strokeDasharray={solid ? 'none' : '4 4'}>
...
export default function SatelliteAssembly({ plan }) {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', height: '100%' }}>
      <svg viewBox="0 0 300 400" style={{ width: '100%', maxWidth: 320, flex: 1, minHeight: 0 }} aria-hidden="true">
        {/* faint star backdrop */}
        {Array.from({ length: 18 }, (_, i) => (
          <circle key={i} cx={(i * 89 + 31) % 300} cy={(i * 47 + 19) % 380} r="0.9" fill={CREAM} opacity=".3" />
        ))}

        {/* antenna — identity (name gives the satellite its callsign) */}
        <Part on={hasAntenna} label="antena uhf" anchor={[150, 108]} labelAt={[226, 96]}
          ghost={<g stroke={CREAM} strokeDasharray="4 4"><line x1="150" y1="150" x2="150" y2="118" /><path d="M132 118 A 24 24 0 0 1 168 118" fill="none" /></g>}>
          <g stroke={CREAM} strokeWidth="1.4" fill="none">
            <line x1="150" y1="150" x2="150" y2="118" />
            <path d="M132 118 A 24 24 0 0 1 168 118" />
            <line x1="150" y1="118" x2="150" y2="106" />
            <circle cx="150" cy="104" r="2.4" fill={GOLD} stroke="none" />
          </g>
        </Part>
        ...
        <Part on={hasPanels} label="painéis solares" anchor={[40, 195]} labelAt={[34, 130]}
          ...
          <line x1="98" y1="195" x2="105" y2="195" stroke={CREAM} strokeWidth="1.4" />
          <line x1="195" y1="195" x2="202" y2="195" stroke={CREAM} strokeWidth="1.4" />
        </Part>
        ...
        <Part on={hasPayload} label="carga útil" anchor={[150, 262]} labelAt={[230, 286]}
          ghost={<g stroke={CREAM} strokeDasharray="4 4" fill="none"><rect x="128" y="240" width="44" height="22" /><circle cx="150" cy="270" r="8" /></g>}>
          <g stroke={CREAM} strokeWidth="1.4" fill="rgba(244,239,230,.05)">
            <rect x="128" y="240" width="44" height="22" />
            <circle cx="150" cy="270" r="8" fill="none" />
            <circle cx="150" cy="270" r="3.4" fill={GOLD} stroke="none" />
          </g>
        </Part>
        ...
        <g style={{ opacity: hasAntenna && plan.name.trim() ? 1 : 0, transition: 'opacity .5s' }}>
          <text x="150" y="200" textAnchor="middle" fontFamily="'Space Mono', monospace" fontSize="11"
            fontWeight="700" fill={GOLD} style={{ letterSpacing: '.12em' }}>
            {plan.name.trim().slice(0, 10).toUpperCase()}
          </text>
        </g>
        {ready && (
          <circle cx="186" cy="161" r="3" fill="#7FD4A0">
            <animate attributeName="opacity" values="1;.25;1" dur="1.6s" repeatCount="indefinite" />
          </circle>
        )}

        {/* engineering drawing title block */}
        <g fontFamily="'Space Mono', monospace" fill={CREAM}>
          <rect x="22" y="346" width="256" height="34" fill="none" stroke={CREAM} strokeOpacity=".35" />
          <line x1="170" y1="346" x2="170" y2="380" stroke={CREAM} strokeOpacity=".35" />
      ...
      <div style={{ ...mono, fontSize: 11, letterSpacing: '.1em', textTransform: 'uppercase', color: ready ? '#7FD4A0' : 'rgba(244,239,230,.55)', paddingBottom: 4, textAlign: 'center' }}>
