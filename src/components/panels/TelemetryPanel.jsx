...
function OrbitViewport({ signal, missionName }) {
  ...
  return (
    <svg viewBox="0 0 500 380" style={{ width: '100%', height: '100%', display: 'block' }} preserveAspectRatio="xMidYMid meet">
      {/* deterministic starfield */}
      {Array.from({ length: 46 }, (_, i) => {
        const x = (i * 103 + 17) % 500, y = (i * 59 + 23) % 380
        return <circle key={i} cx={x} cy={y} r={i % 8 === 0 ? 1.4 : 0.8} fill={CREAM} opacity={0.35} />
      })}

      {/* Earth — simplified poster graphic, not photoreal */}
      ...
      <circle cx="250" cy="196" r="86" fill="none" stroke={CREAM} strokeOpacity=".25" />
      <ellipse cx="250" cy="196" rx="86" ry="30" fill="none" stroke={CREAM} strokeOpacity=".12" />
      <ellipse cx="250" cy="196" rx="44" ry="86" fill="none" stroke={CREAM} strokeOpacity=".12" />

      {/* visible orbit path */}
      <path d={ORBIT} fill="none" stroke={CREAM} strokeOpacity=".4" strokeWidth="1" strokeDasharray="4 6" />

      {/* satellite riding the orbit (plausible LEO-ish period, scaled) */}
      <g>
        <g>
          <rect x="-6" y="-6" width="12" height="12" fill={CREAM} />
          <rect x="-19" y="-3" width="11" height="6" fill={GOLD} />
          <rect x="8" y="-3" width="11" height="6" fill={GOLD} />
      ...
      <g transform="translate(250 352)">
        <path d="M-13 6 A 13 13 0 0 1 13 6 Z" fill="none" stroke={GOLD} strokeWidth="1.8" transform="rotate(-32)" />
        <line x1="0" y1="0" x2="0" y2="14" stroke={GOLD} strokeWidth="1.8" />
        <line x1="-9" y1="14" x2="9" y2="14" stroke={GOLD} strokeWidth="1.8" />
        <text x="18" y="12" fontFamily="'Space Mono', monospace" fontSize="11" fill={CREAM} fillOpacity=".75">
          GS·{(missionName || 'FORGE').slice(0, 10).toUpperCase()}
        </text>
      </g>

      {/* uplink / downlink corridor + traveling signal pulse */}
      <line x1="250" y1="338" x2="250" y2="278" stroke={CREAM} strokeOpacity={signal ? '.35' : '.12'} strokeWidth="1" strokeDasharray="2 5" />
      {signal && (
        <circle r="3.4" fill={signal.dir === 'up' ? GOLD : '#8FC0F0'} key={signal.key}>
          <animateMotion
      ...
      <defs>
        <radialGradient id="earth-shade" cx="34%" cy="30%" r="85%">
          <stop offset="0%" stopColor={CREAM} stopOpacity=".22" />
...
function Sparkline({ data, color, unit, label, value, domain }) {
  ...
  return (
    <div style={{ borderTop: '1px solid rgba(244,239,230,.14)', padding: '8px 0 2px' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline', marginBottom: 2 }}>
        <span style={{ ...mono, fontSize: 11, letterSpacing: '.1em', textTransform: 'uppercase', color: 'rgba(244,239,230,.62)' }}>{label}</span>
        <span style={{ ...mono, fontSize: 13.5, fontWeight: 700, color }}>{value ?? '—'}<span style={{ fontSize: 10, color: 'rgba(244,239,230,.5)', marginLeft: 3 }}>{unit}</span></span>
      </div>
      <svg viewBox={`0 0 ${W} ${H}`} width="100%" height={H} preserveAspectRatio="none" style={{ display: 'block' }}>
        {path
          ? <path d={path} fill="none" stroke={color} strokeWidth="1.4" strokeLinejoin="round" />
          : <text x={W / 2} y={H / 2} textAnchor="middle" fontFamily="'Space Mono', monospace" fontSize="10" fill="rgba(244,239,230,.4)">aguardando…</text>}
      </svg>
    </div>
  )
}
...
function ReadoutRow({ label, value, accent }) {
  return (
    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline', padding: '7px 0', borderBottom: '1px solid rgba(244,239,230,.1)' }}>
      <span style={{ ...mono, fontSize: 11.5, letterSpacing: '.08em', textTransform: 'uppercase', color: 'rgba(244,239,230,.62)' }}>{label}</span>
...
export default function TelemetryPanel() {
  ...
  const charts = [
    entities.bmp280 && { label: 'temperatura', key: 'temp', color: '#E08A5A', unit: '°C', domain: [15, 30] },
    entities.bmp280 && { label: 'pressão', key: 'press', color: '#8FC0F0', unit: 'hPa', domain: [650, 720] },
  ...
  return (
    <div style={{
      ...
      background: 'radial-gradient(130% 130% at 70% 10%, #223255 0%, #18243A 60%, #111B2D 100%)',
    }}>
      {/* console header strip */}
      <div style={{
        flexShrink: 0, display: 'flex', alignItems: 'center', gap: 12, padding: '10px 18px',
        borderBottom: '1px solid rgba(244,239,230,.14)',
      }}>
        <span style={{ ...slab, fontSize: 19, fontWeight: 700, color: CREAM }}>Estação terrestre</span>
        <span style={{ ...mono, fontSize: 11, letterSpacing: '.14em', textTransform: 'uppercase', color: 'rgba(244,239,230,.55)' }}>
        ...
        <span style={{
          ...
          background: real ? 'rgba(46,122,79,.3)' : 'rgba(201,162,39,.18)',
          color: real ? '#7FD4A0' : GOLD, border: `1px solid ${real ? 'rgba(127,212,160,.4)' : 'rgba(201,162,39,.4)'}`,
        }}>{real ? 'hardware real' : 'dados simulados'}</span>
      </div>

      <div style={{ flex: 1, display: 'flex', minHeight: 0 }}>
        {/* ── command console ─────────────────────────────────────── */}
        <div style={{ width: 282, flexShrink: 0, display: 'flex', flexDirection: 'column', borderRight: '1px solid rgba(244,239,230,.14)', padding: '14px 16px', minHeight: 0 }}>
          <div style={{ ...mono, fontSize: 11, letterSpacing: '.16em', textTransform: 'uppercase', color: 'rgba(244,239,230,.55)', marginBottom: 4 }}>
            console de comando
          </div>
          ...
          {COMMANDS.map(c => (
            <button key={c.id} onClick={() => sendCommand(c)} disabled={busy} style={{
              ...
              background: 'rgba(244,239,230,.05)', border: '1px solid rgba(244,239,230,.22)',
              color: CREAM, fontSize: 14, fontFamily: "'Space Grotesk', sans-serif",
              opacity: busy ? .55 : 1, transition: 'all .15s',
            }}
              onMouseEnter={e => { if (!busy) e.currentTarget.style.borderColor = GOLD }}
              onMouseLeave={e => { e.currentTarget.style.borderColor = 'rgba(244,239,230,.22)' }}>
              <span style={{ ...mono, fontSize: 10, color: GOLD }}>▲</span>{c.label}
            </button>
          ))}

          {/* command/ack log */}
          <div style={{ ...mono, fontSize: 10.5, letterSpacing: '.14em', textTransform: 'uppercase', color: 'rgba(244,239,230,.5)', margin: '10px 0 6px' }}>
            log de enlace
          </div>
          <div ref={logRef} style={{ flex: 1, overflowY: 'auto', minHeight: 60, paddingRight: 4 }}>
            {log.slice(-40).map((l, i) => (
              <div key={i} style={{
                ...
                color: l.t === 'tx' ? GOLD : l.t === 'rx' ? '#8FC0F0' : 'rgba(244,239,230,.55)',
        ...
        <div style={{ flex: 1, minWidth: 0, position: 'relative' }}>
          ...
          <div style={{
            ...
            letterSpacing: '.14em', textTransform: 'uppercase', color: 'rgba(244,239,230,.45)',
          }}>órbita ilustrativa · período fora de escala</div>
        </div>

        {/* ── telemetry readout ───────────────────────────────────── */}
        <div style={{ width: 268, flexShrink: 0, borderLeft: '1px solid rgba(244,239,230,.14)', padding: '14px 16px', overflowY: 'auto' }}>
          <div style={{ ...mono, fontSize: 11, letterSpacing: '.16em', textTransform: 'uppercase', color: 'rgba(244,239,230,.55)', marginBottom: 10 }}>
            telemetria {real ? '· tempo real' : '· simulada'}
          </div>
          <ReadoutRow label="bateria" value={`${battery} V`} accent={battery < 7.2 ? '#E08A5A' : undefined} />
          <ReadoutRow label="temp. de bordo" value={`${obcTemp} °C`} />
          <ReadoutRow label="qualidade do enlace" value={`${link} %`} accent={link < 75 ? '#E08A5A' : '#7FD4A0'} />
          <ReadoutRow label="último comando ack" value={lastAck} />
          <ReadoutRow label="payload" value={payloadOn ? 'ATIVO' : 'ESPERA'} accent={payloadOn ? '#7FD4A0' : GOLD} />
          ...
          <div style={{ ...mono, fontSize: 10.5, lineHeight: 1.6, color: 'rgba(244,239,230,.45)', marginTop: 14 }}>
            {real
