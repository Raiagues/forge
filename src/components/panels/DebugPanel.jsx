    ...
    <div style={{ height: '100%', overflow: 'auto', padding: '16px 22px' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'flex-end', justifyContent: 'space-between', marginBottom: 14, gap: 12, flexWrap: 'wrap' }}>
        <div>
          ...
          <span style={{ ...mono, fontSize: 12, color: 'var(--ink4)' }}>ferramentas modulares · estado real do projeto</span>
        </div>
  ...
  return (
    <div style={{ padding: '6px 0', borderTop: '1px solid var(--rule2)' }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 7 }}>
        ...
        <span style={{ fontSize: 13.5, color: 'var(--ink)', fontWeight: 500 }}>{tool.label}</span>
      </div>
      <div style={{ fontSize: 13.5, color: TONE[res.status] === TONE.idle ? 'var(--ink3)' : TONE[res.status], margin: '3px 0 4px 15px' }}>{res.summary}</div>
      <div style={{ marginLeft: 15 }}>
        {res.details.map((d, i) => (
          <div key={i} style={{ display: 'flex', justifyContent: 'space-between', gap: 8, padding: '2px 0' }}>
            <span style={{ ...mono, fontSize: 12, color: 'var(--ink3)' }}>{d.label}</span>
            <span style={{ ...mono, fontSize: 12, color: d.tone ? TONE[d.tone] : 'var(--ink)' }}>{d.value}</span>
          </div>
        ))}
      </div>
    </div>
...
function Card({ title, hint, children }) {
  return (
    <div style={{ border: '1px solid var(--rule)', borderRadius: 7, background: 'var(--paper2)', padding: '10px 12px' }}>
      <div style={{ ...mono, fontSize: 11, letterSpacing: '.12em', textTransform: 'uppercase', color: 'var(--ink4)' }}>{title}</div>
      {hint && <div style={{ fontSize: 13, color: 'var(--ink3)', marginTop: 2 }}>{hint}</div>}
      {children}
    </div>
  )
}
...
function Badge({ tone, children }) {
  ...
  return <span style={{ ...mono, fontSize: 11, letterSpacing: '.06em', padding: '3px 8px', borderRadius: 3, border: `1px solid ${s.border}`, background: s.background, color: s.color }}>{children}</span>
}
