...
function Finding({ f, rating, onRate, onFix }) {
  return (
    <div style={{
      ...
      <div style={{ fontSize: 11, color: 'var(--ink2)', lineHeight: 1.55, marginBottom: 6 }}>{f.cause}</div>

      {f.evidence?.length > 0 && (
        <div style={{ marginBottom: 7 }}>
          {f.evidence.map((e, i) => (
            <div key={i} style={{ display: 'flex', gap: 6, padding: '1px 0' }}>
              <span style={{ ...mono, fontSize: 9, color: 'var(--ink4)', flexShrink: 0 }}>›</span>
              <span style={{ ...mono, fontSize: 9.5, color: 'var(--ink3)', lineHeight: 1.5 }}>{e}</span>
            </div>
          ))}
        </div>
      )}
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 6, flexWrap: 'wrap' }}>
        {f.fixes?.map((fix, i) => (
          <button key={i} onClick={() => onFix(fix, f.title)} style={{
            fontSize: 10.5, cursor: 'pointer', padding: '4px 10px', borderRadius: 4,
        ...
        {rating == null ? (
          ...
          <span style={{ ...mono, fontSize: 8.5, color: rating ? 'var(--ok2)' : 'var(--err2)' }}>
...
function rateBtn(color) {
  return {
    ...mono, fontSize: 8.5, letterSpacing: '.05em', textTransform: 'uppercase',
    ...
    <div>
      <textarea
        ...
        style={{
          ...
          ...mono, fontSize: 10.5, lineHeight: 1.6,
        }}
      />
      <div style={{ display: 'flex', gap: 6, marginTop: 7, flexWrap: 'wrap' }}>
        <button onClick={() => runLogDoctorOnText(text, 'paste')} disabled={running} style={{
          ...
          background: 'var(--navy)', color: 'rgba(255,255,255,.88)', fontSize: 11.5,
          fontFamily: "'Space Grotesk', sans-serif",
